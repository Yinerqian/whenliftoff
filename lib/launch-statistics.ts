...
export const HOME_STATISTICS_ID = "rolling-12-complete-months";

export function completeUtcMonthRange(now = new Date()) {
  const end = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));
  const start = new Date(Date.UTC(end.getUTCFullYear(), end.getUTCMonth() - 12, 1));
  return { start: start.toISOString(), end: end.toISOString() };
}

function monthKeys(start: string, count = 12) {
...
export function aggregateHomeLaunchStatistics(
  ...
  const monthly = new Map(monthKeys(periodStart).map((month) => [month, { month, total: 0, successful: 0 }]));
  const providers = new Map<string, { name: string; abbrev: string | null; count: number }>();
  const countries = new Map<string, { code: string; name: string; count: number }>();
  const rockets = new Map<string, number>();
  ...
  for (const launch of launches) {
    ...
    if (rocket) rockets.set(rocket, (rockets.get(rocket) ?? 0) + 1);
  ...
  const countryStats = topWithOther<HomeCountryLaunchStat>(
    ...
    7,
    (count) => ({ code: "OTH", name: "其他", count, share: percentage(count, total), is_other: true }),
  );
  const rocketStats = topWithOther<HomeRocketLaunchStat>(
    [...rockets].map(([name, count]) => ({ name, count, share: percentage(count, total) })),
    7,
    (count) => ({ name: "其他", count, share: percentage(count, total), is_other: true }),
  );
